...
var app  = new Framework7({
  ...
  id: 'io.framework7.bhagawadgita-id', // App bundle ID
  name: 'Bhagawad Gita', // App name
...
var ac_share = app.actions.create({
  buttons: [
    ...
    '</div></li></ul></div>',
      onClick: function () {
        var msg = 'Download aplikasi Bhagawad Gita di Playstore.\n\n' +
        'https://play.google.com/store/apps/details?id=com.app.bhagavadgita-id';
        window.plugins.socialsharing.shareViaWhatsApp(msg, null, null, null, function(e){
          app.dialog.alert('Sharing failed with message: ' + e, 'Bhagawad Gita');
        })
      }
    },
    {
    ...
    '</div></li></ul></div>',
      onClick: function () {
        var msg = 'Download aplikasi Bhagawad Gita di Playstore.';
        window.plugins.socialsharing.shareViaFacebookWithPasteMessageHint(msg, null, 'https://play.google.com/store/apps/details?id=com.app.bhagavadgita-id', 'Paste pesan anda!', null, function(e){
          app.dialog.alert('Sharing failed with message: ' + e, 'Bhagawad Gita');
        })
      }
    },
  ...
  if (app.views.main.router.url == '/') {
    ...
    if (isRated !== 'Y') {
      app.dialog.confirm('Jika anda menyukai aplikasi ini, mohon luangkan waktu anda untuk memberi rate aplikasi di Play Store.', function () {
        ...
        navigator.app.loadUrl("https://play.google.com/store/apps/details?id=com.app.bhagavadgita-id", {openExternal : true}) //android
